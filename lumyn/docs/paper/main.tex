...
%
% NOTE ON VERIFICATION: this file has now been COMPILED.  The authoring
% environment had no LaTeX toolchain, so the rendered output was unverified for
% a long time; it was built on 2026-09-12 with TeX Live on Ubuntu 22.04 via
% latexmk.  Result: 10 pages, 0 undefined references, 0 Overfull/Underfull boxes.
% The first compile reported six Overfull \hbox warnings, all of them in table
% headers -- the fixes are the \tabcolsep reduction below and \small /
% \footnotesize on the widest tables.  `docs/paper/check_tex.py` remains useful
% as a structural check and is complementary to an actual build.
...
% Column separation tightened from the 6pt default: the first compile (on a machine
% with TeX, which the authoring environment lacked) reported six Overfull \hbox
% warnings, all of them in table headers. Reducing the inter-column space is the
% least invasive fix and keeps every table in the same style.
...
\centering
\small
% [fit] table font reduced to \small; see build note in preamble
...
\centering
\footnotesize
% [fit] \footnotesize: this table is the widest in the paper
